...
class SidebarHelper extends Helper {
    ...
    public $singleSpanClasses = "d-flex align-self-center g-pos-rel g-font-size-18 g-mr-8";
    ...
    public $mainSpanClasses = "d-flex align-self-center g-pos-rel g-font-size-18 g-mr-8";
    ...
    public $subAnchorClasses = "media u-side-nav--second-level-menu-link g-px-0 g-py-8";
        ...
        if (!empty($menuItems)) {
            ...
            <script>
                $(function () {
                    ...
                    var _this = $('[id*="nav_<?= $this->getView()->getRequest()->getParam('controller') . $this->getView()->getRequest()->getParam('action'); ?>"]');
    ...
    public function menuItem($menuItem, $index) {
        ...
        if (!$item['custom_sub_menu'] && !$item['default_sub_menu']) {
            ...
            $id = 'nav_' . $item['controller'] . $item['action'];
            if (empty($item['slug'])) {
                $url = Router::url(['controller' => $item['controller'], 'action' => $item['action']]);
            } else {
                $url = Router::url(['controller' => $item['controller'], 'action' => $item['action'], $item['slug']]);
            }
            ?>
                ...
                <ul id="<?= $mainId; ?>" class="<?= $this->subUlClasses; ?>">
                    ...
                    <?php if ($item['default_sub_menu']) {
                        foreach ($item['default_sub_menu_items'] as $subItem) {
                            ...
                            $id = 'sub_nav_' . $subItem['controller'] . $subItem['action'];
                            if (empty($subItem['slug'])) {
                                $url = Router::url(['controller' => $subItem['controller'], 'action' => $subItem['action']]);
                            } else {
                                $url = Router::url(['controller' => $subItem['controller'], 'action' => $subItem['action'], $subItem['slug']]);
                    ...
                    if (!empty($item['custom_sub_menu'])) {
                        foreach ($item['custom_sub_menu'] as $subItem) {
                            $subItem = $this->buildItem($subItem);
                            $id = 'nav_' . $subItem['controller'] . $subItem['action'];
                            if (empty($subItem['slug'])) {
                                $url = Router::url(['controller' => $subItem['controller'], 'action' => $subItem['action']]);
                            } else {
                                $url = Router::url(['controller' => $subItem['controller'], 'action' => $subItem['action'], $subItem['slug']]);
